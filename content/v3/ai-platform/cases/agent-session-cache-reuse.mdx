...
description: "Синтетический пример: два одинаковых агентных запроса различаются только порядком инструментов, а закреплённая версия анализатора обнаруживает это изменение."
...
После слияния черновика в `main` канонические ссылки будут вести на [стабильный запрос](https://github.com/sernote/sernote.github.io/blob/main/evidence/v3/agent-session-cache-reuse/step-stable.json), [запрос с перестановкой инструментов](https://github.com/sernote/sernote.github.io/blob/main/evidence/v3/agent-session-cache-reuse/step-drift.json) и [машинный снимок двух запусков](https://github.com/sernote/sernote.github.io/blob/main/evidence/v3/agent-session-cache-reuse/layout-linter-output.json). В снимке сохранены исходные команды, отдельный переносимый рецепт, ожидаемые и полученные коды завершения и оба JSON-результата.
