...
In a multi-turn agent, changing the tool subset on each step may reduce raw prompt length but reset the cacheable prefix. A stable tool declaration with route-level allowed tools or constrained decoding can preserve reuse.
