...
Release preparation, 5 September 2026: the completed editorial discovery, handbook entrance and cache-series work is ready for a feature-branch push from `codex/cache-editorial-exemplar` and a draft PR against `main`. A fresh source tree based on `c763c7a3885eef67530d5a4e14cc1bf8c9066e81`, containing only the intended PR changes, passed a frozen-lockfile install and the full `pnpm verify`: lint, typecheck, 408 tests, a 117-page build, 46 production export tests and both export audits. The independent cumulative review found no actionable blocker. The prior local hybrid-reasoners draft and local settings remain outside the release scope; the staged test contract uses the existing published article. The current workflow deploys only on a `main` push or manual dispatch, so the feature push and draft PR do not publish the site or run that workflow. See `.agent/SITE_RELEASE_QA_2026-09-05.md`. Earlier local/uncommitted notes below record milestone-time state.
