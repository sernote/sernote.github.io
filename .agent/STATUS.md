...
Completed locally, 23 September 2026: a new native Blog article, «Поменял effort — куда делся кэш?», expands the author's Telegram post 670 into a source-backed explanation of effort changes and prompt-cache reuse. It corrects the post's turn-duration claim against current OpenAI and Anthropic documentation, distinguishes API mechanisms from client support and measured cache reads, and uses two restrained `show-me` diagrams. A Claude Opus 5.5 xhigh editorial pass through Porch informed the final Russian prose. The canonical route is registered for static export, RSS and sitemap. On current `origin/main`, full `pnpm verify` passed: lint, typecheck, 419 tests, a 118-page static build, 46 production export checks and both audits. Work is isolated on `codex/effort-cache-article`; the original checkout and its local changes are untouched. No deployment has been performed.

The readable Home correction approved on 6 September 2026 is implemented and verified on `codex/home-reading-layout`, based on deployed `0aea379`. Home now has a compact author introduction, one article column, and direct handbook/project entrances. Independent spec and code reviews passed; `pnpm verify` passed with 419 tests, 117 pages and 46 export integration checks. Browser checks at 1,440 / 768 / 390 px confirmed readable text, no horizontal overflow and working reader paths. Public text/link fingerprints changed only on Home across 115 exported HTML files. Evidence: `.agent/HOME_READING_LAYOUT_QA_2026-09-06.md`; governing design and plan: `docs/superpowers/` with the `2026-09-06-home-reading-layout` prefix. Delivery is tracked by the pull request for this branch and the Pages workflow on `main`. The original checkout's drafts remain untouched.

Publication of the completed A/B/C release was explicitly authorized on 6 September 2026. Release delivery is tracked by the pull request for `codex/site-practical-first-release` and the `Deploy to GitHub Pages` workflow on `main`; the local-only statements below record earlier milestones.

Approved A/B/C practical-first release complete locally on `codex/site-practical-first-release`. Home and Blog now surface existing articles without repeated journeys; the handbook has a direct task table and responsive TOC; public labels/roles and audit outcomes are accurate; the four-article series and archived-auditor continuation work. Independent reviews are complete; the discovered CSS cascade and fallback-lead duplication issues are fixed and verified. Final `pnpm verify` passes: 415 tests, 117 pages, 46 production export checks, both audits. Desktop/mobile/tablet browser checks and public first-audit reproduction are recorded in `.agent/SITE_PRACTICAL_RELEASE_QA_2026-09-06.md`. All three stages are complete; no further cache polishing is required for this release. Changes remain local, with no push, merge or publication. The original checkout and user draft remain untouched.

Stage B is complete locally: practical task table and three first-audit outcomes, responsive reference TOC with keyboard navigation, preserved editorial anchors. Browser QA found and resolved a cross-chunk CSS cascade conflict; desktop/mobile/tablet behavior verified. Public text fixtures reproduce 43 → 254 bytes; no model performance measurement is claimed. Independent reviews, fresh lint/typecheck, 20 focused tests and build pass. Continue authorized stage C; details in `.agent/SITE_PRACTICAL_RELEASE_QA_2026-09-06.md`. No publication or merge.

Stage A of the approved practical-first release is complete locally on `codex/site-practical-first-release`: Inference Plane copy and legacy anchors, participant bylines, channel labels and accurate public actions, restrained review metadata. Independent content review finding (map badge) is fixed; separate code review has no findings. Fresh lint, typecheck, 50 focused tests and build pass. All previous reference IDs are preserved; desktop/mobile browser inspection completed. Plan: `docs/superpowers/plans/2026-09-06-site-practical-first-release.md`; receipt: `.agent/SITE_PRACTICAL_RELEASE_QA_2026-09-06.md`. Continue authorized B/C. No site publication or merge.

Completed authorized Claude Fable 5.1 review of the site plan, 6 September 2026. The `review ask` run used the requested `claude-fable-5-1` model at medium effort and completed successfully with exit 0. Its public-page packet and original proposal excluded the talk source files and the self-review. Full opinion: `.agent/reviews/SITE_PLAN_REVIEW_FABLE_2026-09-06.md`; decisions and revised stages: `.agent/SITE_PLAN_REVIEW_2026-09-06.md`. Fresh live HTML checks confirmed that the three cited archive pages already have `noindex, follow`, an archive notice and a current-section link; archive work is a P2 discovery improvement, not an indexing blocker. The synthesis keeps the smaller A/B/C sequence, clearer diagnostic outcomes, restrained review labels and reuse of existing illustrations. It rejects HTML grep as a substitute for reader/visual checks, mandatory photo/series ordering, and the claim that the shared TOC affects every exported page. Fresh `pnpm lint`, `pnpm typecheck` and `pnpm build` passed; log: `/private/tmp/notevskii-site-plan-review-2026-09-06/consilium-validation.log`. No site implementation or publication was performed.

Completed self-review of the site proposal, 6 September 2026: `.agent/SITE_PLAN_REVIEW_2026-09-06.md` reviews the plan at `9acc1ab` against the current implementation and pinned public project instructions. Six findings narrow the first release, distinguish audit outcomes, preserve the personal publication, treat layout choices as hypotheses, include the heading validator in editorial changes, and define when to move beyond caching. The proposed sequence is exact corrections, one practical exemplar, then improved discovery. This is a review artifact, with no site implementation or publication. At the self-review stage, the new external Claude review was not run because automatic approval review rejected the payload as outside previous consent. The user subsequently granted consent and the completed follow-up is recorded above. Fresh `pnpm lint`, `pnpm typecheck` and `pnpm build` passed; log: `/private/tmp/notevskii-site-plan-review-2026-09-06/validation.log`. The original checkout and user draft remain untouched.
