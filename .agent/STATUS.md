...
The v3 implementation remains the technical baseline, but it is no longer the target product. The accepted v3.1 correction spec, exact route decisions, style gate 4a, and fifth-iteration visual package have converged with no open P0/P1/P2. The implementation target now covers Home, Blog, Materials, AI Platform, and About; detail and reference pages will be derived through one reviewed implementation exemplar per type. V3.1 implementation has not started, and the reviewed implementation plan is the next gate. On 2 August 2026 the conflicting GitHub Pages deployment for `sernote/blog` was disabled without deleting the repository; the GitHub API now returns 404 for that Pages resource, while the public `/blog/` edge still served the old cached Jekyll page during the immediate follow-up check. External production smoke remains pending cache expiry.
...
- Diagnosed the live `/blog/` collision to the legacy Pages resource in `sernote/blog` (`main` root, Jekyll, project path `/blog/`) and disabled only that Pages resource. The repository and its files remain intact and Pages can be re-enabled if needed. The main `sernote/sernote.github.io` Pages deployment remains built with `notevskii.tech`, workflow publishing, and HTTPS enforcement.
