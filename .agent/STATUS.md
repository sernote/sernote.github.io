...
The v3 implementation remains the technical baseline, but it is no longer the target product. The accepted v3.1 correction spec, exact route decisions, style gate 4a, fifth-iteration visual package, and executable v3.1 implementation plan have converged with no open P0/P1/P2. The target covers Home, Blog, Materials, AI Platform, and About; detail and reference pages will be derived through one reviewed implementation exemplar per type. V3.1 implementation is the current milestone and has started: Task 1 is complete and Task 2 is next. The separate GitHub Pages deployment for `sernote/blog` was disabled on 2 August 2026 without deleting the repository. After the edge cache expired, external smoke returned the main site's `/blog/` with HTTP 200 and title `Блог — Сергей Нотевский`; the operational incident is resolved.
