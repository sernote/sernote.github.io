...
- Added the native Blog article «Гибридная reasoning-модель удобна. Но нагрузку всё равно придётся разделять», adapted from Sergey's draft and published on 21 August 2026. The article keeps the first-person production argument while correcting two factual boundaries against the official Qwen and vLLM sources: Qwen3-2507 shipped separate Instruct/Thinking checkpoints but Qwen3.5 later returned to switchable thinking, and the vendor AIME25 gains are reported as a cross-release comparison rather than a controlled hybrid-vs-separate ablation. The serving section distinguishes checkpoint choice from pool topology, explains the mixed prefill/decode trade-off without claiming that every workload requires physical separation, and keeps all internal company details out. TDD RED reproduced the missing article and route; focused GREEN passed 82/82 tests. Claude Opus and Sonnet consilium reviews were attempted read-only but both were rejected before reading the text by the configured Claude Team account with `403 Request not allowed`; the same editorial and factual pass was completed locally and the limitation is recorded rather than hidden. Final validation passed ESLint, TypeScript, 22 Vitest files / 373 tests with 6 intentional skips, the 113-page static build, 44 production export tests, the 6-file/16-transition reference audit and the static export audit (108 routes, 35 aliases, 54 archives, 19 sitemap URLs). Export inspection confirmed the new article on Home and Blog, 14 RSS items, one H1/main/canonical on the detail page and valid RSS/sitemap XML.
