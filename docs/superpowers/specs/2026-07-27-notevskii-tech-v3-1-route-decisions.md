...
Этот документ задаёт судьбу каждого маршрута из текущего v3 export и добавляет новый канонический `/materials`. Живой production проверяется отдельным внешним crawl и может расходиться с этим снимком.
...
- `archive` — сохранённая страница с HTTP 200, self-canonical и `noindex,follow`. Не входит в навигацию, sitemap, RSS, featured и relations.
...
- дата последней известной редакции;
...
2. `/materials` добавлен как единственный новый статический верхнеуровневый маршрут;
