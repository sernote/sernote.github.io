export const AUTHOR_PROFILE = Object.freeze({
  id: "https://notevskii.tech/about/#person",
  name: "Сергей Нотевский",
  role: "AI Platform Lead",
  company: "Битрикс24",
  url: "https://notevskii.tech/about/",
  sameAs: Object.freeze([
    "https://habr.com/ru/users/Ser_no/",
    "https://github.com/sernote",
    "https://t.me/sergeinotevskii"
  ]),
  aboutIntro:
    "Я руковожу направлением AI Platform и командой в Битрикс24. Мы делаем платформу для AI-сценариев продуктов компании и поддерживаем её в работе.",
  currentWork:
    "Мы занимаемся инфраструктурой инференса, маршрутизацией и очередями, качеством, наблюдаемостью, мощностями и стоимостью. Я отвечаю за технические решения и работу команды.",
  career:
    "До перехода в продуктовую роль я уже несколько лет работал на технической стороне продукта. После выхода ChatGPT начал разрабатывать и внедрять AI-сценарии. В 2024 году к этой работе добавились продуктовая роль и AI-евангелизм в Битрикс24. В 2025-м я перешёл в разработку CoPilot, а сейчас руковожу AI Platform.",
  sitePurpose: Object.freeze([
    "Здесь я разбираю задачи, с которыми сталкиваюсь в работе: локальный инференс, устройство контура управления, качество моделей и агентов, планирование мощностей, стоимость и кэширование.",
    "Одни темы становятся статьями или докладами. Другие превращаются в код: так появился audit-prompt-caching. Большие разборы постепенно складываются в раздел AI Platform."
  ]),

  channelName: "AI да парень!",
  channelPitch:
    "В Telegram пишу короче и чаще: делюсь наблюдениями из работы, новыми находками и продолжениями разборов.",
  organizerNote:
    "Биография и портрет для анонса — на этой странице. Записи выступлений и интервью собраны в разделе «Материалы»."
});
